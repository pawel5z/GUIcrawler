...
class CrawlResult:
    def __init__(self, startAddress, results):
...
def crawl(startPage, maxDepth, action):
    toVisit = queue.Queue()
    ...
    while toVisit.empty() == False:
        ...
        for i in range(min(downloaded.qsize(), 8)):
            t = threading.Thread(target=processSite, args=(toVisit, downloaded, visited, actionRes, maxDepth, action, l))
            t.start()
            threads.append(t)
        for t in threads:
            t.join()
    ...
    return CrawlResult(startPage, actionRes)
